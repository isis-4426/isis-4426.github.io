async function drawScatter() {
  ...
  // set data constants
  const xAccessor = d=>+d.TensionArterialSistolica
  ...
  const wrapper = d3.select("#tp1-v4-svg")
    .append("svg")
  ...
  const xAxisLabel = xAxis.append("text")
      ...
      .html("Tension arterial Sistolica")
  ...
  const legendGroup = bounds.append("g")
      .attr("transform", `translate(${
        dimensions.boundedWidth - dimensions.legendWidth + 80
      },${
  ...
  const tooltip = d3.select("#tooltip-tas")
  ...
  function onVoronoiMouseEnter(voronoiDatum) {
    ...
    tooltip.select("#date")
        .text(formatDate(dateParser(datum.Fecha)))
